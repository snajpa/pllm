...
def build_prompt(mission, history, console_state, options, iteration_n)
  ...
  Please analyze the console state, mission, and instructions to suggest key presses that will help the user progress towards completing the mission.
  ...
  - You are guiding the user through an interactive console session. Carefully identify the current state and provide key presses to progress accordingly.
  ...
  - Find the latest output in the console and attend to it. All the previous output is history by now, which you can use to inform yourself, but you should act on the most recent lines only.
  ...
  Your response based on the current console state, mission, and mission history:
  ```json
  PROMPT
end
